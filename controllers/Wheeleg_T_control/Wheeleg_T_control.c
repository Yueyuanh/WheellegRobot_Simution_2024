...
int main(int argc, char **argv) {
   ...
   */
   ...
   // //获取陀螺仪数据
    ...
    //   (void)wb_camera_get_image(camera);
     ...
     //controllers init
    pid_type_def SPEED_PID,LEG_SPEED_PID,BALANCE_PID,TURN_PID,ROLL_PID;
    
    
    const float speed_pid[3]     ={0.012,0,0.0003};
    const float leg_speed_pid[3] ={0.01,0.1,0};
    const float balance_pid[3]   ={400,2,0};
    const float turn_pid[3]      ={20,0,1};
    const float roll_pid[3]      ={3,0,5};
    // const float k1=-300;
    // const float k2=-8; 
    
    PID_init(&SPEED_PID,speed_pid,0.55,0.1);
    PID_init(&LEG_SPEED_PID,leg_speed_pid,0.2,0.1);
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
     ...
     */
     ...
     //leg length
     //leg_length_L=1.7,leg_length_R=1.7;
    ...
    /*********************************************************************************/
     ...
     // //speed loop   : PI control
      ...
      balance_motor_current=PID_calc(&BALANCE_PID,pitch,0.0);
     ...
     /*************************************************************************/
      ...
      //printf("leg_length_R: %f leg_length_L: %f",leg_length_R,leg_length_L);
      printf("leg: %f",leg_buff_l);
      ...
      wb_motor_set_position(rfLeg,leg_length_R+leg_buff_r+wheelBuff);
      wb_motor_set_position(rbLeg,-leg_length_R-leg_buff_r+wheelBuff);
      wb_motor_set_position(lfLeg,leg_length_L+leg_buff_l+wheelBuff);
      wb_motor_set_position(lbLeg,-leg_length_L-leg_buff_l+wheelBuff);
     }
  };
...
void Keyboard_control()
{
    ...
    switch(Key)
    {
      case 87: go_back=-75;break;
      case 83: go_back=75;break;
      case 65: left_right=-0.05;break;
      case 68: left_right=0.05;break;
      case 81: zero_force=1;break; //q
      //case 69:;break; //e
    ...
    if(Key==32)
    {
        leg_length_L=0.8,leg_length_R=0.8;
    }
    else{
        leg_length_L=1.7,leg_length_R=1.7;
    }
